...
| **ClickToCollect (iOS, Vegas / Lexi)** | **Canonical Cursor + Xcode tree:** `~/Desktop/ClickToCollectApp/` — **git repo initialized 2026-04-20** at that root (not inside **ClickToClaim**). App sources: `ClickToCollect/ClickToCollect/*.swift`, **`CLAUDE.md`**, **`TOMORROW.md`**, **`SETUP.md`**. **2026-04-21:** Progress log + **SETUP §5 (TestFlight)** capture Collection/search polish, **`photoContentLabels`** optional migration fix, commit **`31f5db2`** on **`main`**. **2026-05-06:** Labs tab **hidden on App Store production** (`labsTabAllowedForInstall` + sandbox receipt for TestFlight); import board **sparkle FX** tuning; Labs **DEBUG delete-all**; see **`CLAUDE.md`** progress log. **2026-05-08:** Pin delete safety (**dismiss → delete by ID**, notifier refresh), **boards** launch overlay perf (scoped fetch + gate), sticky Collection chrome (**`safeAreaInset`** + **`.searchable` always**), pin landing (**pan when zoomed**, toolbar / **`PinStandardQuickTags`** / **`#DEBUG:`** strip), crop editor scale vs zoom — details in Desktop **`CLAUDE.md`** → *Progress log*. **2026-05-08 (eve):** **`customTags`** migration **no SQL `COUNT` predicate**; import sparkles (**full-bleed processing**, **`softLight`** on photo FX, outline-only box trace); **Find Similar Pins** toolbar leading vs **Done**; **Trade**/**Sell** filters; **`AppFeatureFlags`** post-import tag sheet default off — see **`CLAUDE.md`** *Progress log (short)*. **2026-05-12:** Optional **Core ML** crop classifiers (**character / hidden Disney / manufacturer**) + `.mlpackage` bundles in-repo; post-import runners **default off** (`UserDefaults` unset → no auto ML tags); character path adds **crop inset**, **top‑1 vs top‑2 margin**, **headline allowlist**; **Search** missing scroll helper fixed; **Cleanup** drops **Small**, uses **sheet** for pin detail; **Collection** **Remove tags** + confirm; **Labs** pricing **delete all** index order + library wipe notifier; **SwiftData** re-fetch **`Pin`** after **`await`** before **`customTags`** (fixes detach fatal during **re-detect visible area**). **2026-05-12 (follow-on):** Duplicate sheet **felt color gate** on layout-only pairs; post-import **no board montage** + deferred scan after **See my boards & pins**; **View on board** link; **Carded** / library scan **streaming grid** + scroll fixes; **board zoom pan clamp**; crop-exit **zoom reset**. Details: Desktop **`CLAUDE.md`** *Progress log (short)*. **2026-05-14:** PinVlm / Labs **v1** scope and **K-way / abstention** learnings summarized in **this repo** **`FUTURE.md`** (*Session log — 2026-05-14*); app commits on **`main`** include **`47edd9a`** (MyCollectionView types), **`8ef2c5f`** (Labs thumbs), **`b05eddc`** line (Swift 6 explicit `service:`), **`PinCanonicalDuplicateLinking`** `let desc` cleanup. **ClickToClaim `FUTURE.md`:** cross-photo / same-wall overlap cleanup (v2/v3 roadmap). **2026-05-15:** `main` @ **`7290237`** — `CURRENT_PROJECT_VERSION` **6**; PinVlm out of Labs + model dropped; Collection/Cleanup/Add copy trims; multi-select **OR** filters; Overlap progress/refresh; **SearchView** filter API; board delete **Don’t Delete** confirmed. Session log: **`FUTURE.md`** (*Session log — 2026-05-15*). **Security (v1 vs App Store):** **`FUTURE.md`** → *ClickToCollect — security (v1 vs App Store)* (2026-05-16). **2026-05-16:** `main` @ **`0bb892c`** — View on Board, filter UX, cleanup progress overlay, quick actions/Spotlight, security phases + archived models + Swift 6 fix; version **7**; session log **`FUTURE.md`** (*Session log — 2026-05-16*). **2026-05-18:** detection passes + cleanup tools redesign **proposed** (provenance/quality flags, `isSearchablePin`, density-hotspot Pass C, Duplicates same-board vs cross-board split) — see **`FUTURE.md`** → *ClickToCollect — detection pipeline + cleanup tools redesign (proposed) — 2026-05-18*. |

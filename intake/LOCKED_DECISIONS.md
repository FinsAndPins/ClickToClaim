...
| Moderation | One cloud vendor (Sightengine wired first). Reject → delete image, alert with identity + reason codes only. Second vendor later. |
...
| Staff | Steve + Lexi. Cloudflare Access. |
| URLs | `sell.finsandpins.shop` (not a path on the Shopify shop). |
| Email | From `offer@finsandpins.shop`. Alerts to `finsandpins@gmail.com` (+ Lexi). |
